...
`counterRate`, `rate`, `delta`, `counterDelta`, `movingAvg`, `movingMin`, `movingMax`, `deadband`, `hysteresis` хранят состояние keyed по `objectPath|targetVariable`. На сервере ISPF состояние **персистентно** в системной переменной `@bindingState` (JSON на объект, скрыта в UI) — переживает restart и общее для реплик с одной БД.

Ограничения v1:

- Timed window samples обрезаются по окну и cap **256** samples на binding target.
- Multi-replica без distributed lock: last-write-wins при одновременной записи.
- NATS/event bus **не** синхронизирует binding state между JVM — только shared PostgreSQL.

В unit-тестах `ispf-expression` по умолчанию используется in-memory port (`InMemoryBindingStatePort`).
