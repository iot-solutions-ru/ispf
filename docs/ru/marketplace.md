...
Маркетплейс по умолчанию: [ispf-marketplace](https://github.com/Michaael/ispf-marketplace) (настраиваемый). Вендоры могут разместить собственный совместимый сервер.
...
Совместим с API [ispf-marketplace](https://github.com/Michaael/ispf-marketplace):

- `GET /api/v1/catalog` → `{ listings: [...] }`
- `GET /api/v1/catalog/{slug}/download` (free)
- `POST /api/v1/entitlements/activate` (paid)

Поля листинга, используемые UI: `slug`, `title`, `description`, `pricing`, `appId`, `vendorName`, `vendorLegalName`, `vendorInn`, `vendorSellerKind` (`company` | `individual`), `vendorContactPerson`, `vendorContactEmail`, `vendorContactPhone`, `priceCents`, `latestVersion`, `minIspfVersion`.
...
| 4 | Проверка подписи bundle при install | Запланировано |
| 5 | Version pinning + upgrade path (`latestVersion`, semver) | Частично |
| 6 | Проверка `minIspfVersion` перед install | Частично |
...
| 10 | 10+ подписанных production bundle | Запланировано |
...
На **marketplace VPS** (`ispf-marketplace.iot-solutions.ru`):
...
ISPF только проксирует download; исправление всегда на хосте маркетплейса. См. [ispf-marketplace/deploy/README.md](https://github.com/Michaael/ispf-marketplace/blob/main/deploy/README.md).

## См. также

- [COMPETITIVE_SCORECARD.md](competitive-scorecard.md) — измерение 12 (экосистема / маркетплейс)
