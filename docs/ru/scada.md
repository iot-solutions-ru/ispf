...
> **Статус:** Stable — Mimics, symbols, bindings. Хаб: [doc-status.md](doc-status.md).

Конфигурируемые мнемосхемы / P&ID / однолинейные схемы с каталогом SVG-символов, live-привязками к переменным дерева объектов и управлением с HMI.
...
Мнемосхема SCADA в ISPF закрывает типичные задачи операторского HMI:

| Задача | Механизм |
|--------|----------|
| Однолинейная / технологическая схема | Документ `diagramJson` + символы |
| Live-отображение уровней, состояний клапанов, аварий | `bindings` на переменные устройства/модели |
...
Логика остаётся в **дереве объектов** (устройства, привязки, функции). Мнемосхема — **представление**, не отдельный runtime.
...
|---------|-----------|--------|---------------|
| **Объект платформы** | Дерево `root.platform.*` | `DEVICE`, `DASHBOARD`, **`MIMIC`** | Explorer, API, bundle, agent tools |
| **Элемент диаграммы** | Внутри `diagramJson` | tank, valve, label | Редактор → инструмент **Place** |
| **Данные на схеме** | Переменные объекта платформы | `level`, `open`, `alarm` | Explorer → устройство/модель; на схеме только **binding** |

Редактор мнемосхемы **не создаёт** устройства и не пишет телеметрию — он рисует символы и привязывает **`ref`** (PlatformRef slash-адрес) к свойствам символа.
...
| Поле | Описание |
|------|----------|
| `mimicPath` | Путь к объекту `MIMIC` — диаграмма загружается с сервера (**рекомендуется** для переиспользования) |
...
Пример фрагмента layout (сетка дашборда **84×8**):
...
![Редактор мнемосхемы SCADA — магистральный нефтепровод (P&ID) и библиотека символов](../assets/ispf-scada-snmp.png)
...
|--------|--------|---------|
| **Transform** | Flip H/V, rotate ±90° | ≥ 1 выделенный элемент |
| **Align** | left / center H / right / top / middle V / bottom | ≥ 2 элемента |
| **Distribute** | равномерно по H / V | ≥ 3 элемента |
| **Grid** | показать сетку, snap to grid (toggle) | всегда |

Выравнивание выполняется **внутри bounding box выделения** (как в Visio/Figma). После align/distribute и перемещения символов линии **пересчитываются** из портов.
...
| Smart-snap | При перетаскивании края/центры/порты «липнут» к другим элементам (~10 px); пунктирные направляющие на холсте |
...
- Поля **W / H** в панели свойств синхронизированы с ручками.

> Рамка выделения — **логический bbox** символа (`symbolSize()`). У части символов (например, vertical tank) рисунок не заполняет всю область — подписи вроде «T» и «— / 100» входят в bbox.

### Панель свойств

Координаты **X / Y**, размер **W / H**, поворот, слой, **bindings** (в т.ч. `qualityField`), **format rules**, **actions** (несколько на элемент: primary / context), подсказки (`tooltip`), custom SVG. Панель **Layers** — видимость, lock, активный слой для Place.

### Соединения

- Линии крепятся к **портам** символов (`from` / `to`: `elementId` + `port`).
- Маршрут **ортогональный**; при перемещении или повороте символа путь **пересчитывается** от текущих позиций портов.
...
|-----------|-----|----------|
| **pack-valves**, **pack-pumps**, **pack-tanks**, **pack-pipes**, **pack-instruments**, **pack-electrical**, **pack-isa**, **pack-misc** | `pack.ispf-pid.*` | Стандартный P&ID-pack ISA/ISO (**218** символов), статическая геометрия |
...
1. **Стандартный символ** — Place из категории pack → `symbolId: pack.ispf-pid.*`, bindings в панели свойств.
2. **Кастомизация pack** — выделить элемент → «Edit as SVG» → правка разметки → **«Update symbol in library»** → символ появляется в «Custom SVG».
3. **Загрузка SVG** — категория «Custom SVG» → «Upload SVG» → сразу `inUserLibrary: true`.
4. **Import/Export** — полный `diagramJson` в панели импорта (удобно для seed и CI).
...
- Bindings мнемосхемы используют те же переменные, что и виджеты chart/value/function.
