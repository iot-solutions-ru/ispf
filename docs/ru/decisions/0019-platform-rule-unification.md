> **Язык:** русская версия (вычитка). Канонический английский: [en/decisions/0019-platform-rule-unification.md](../../en/decisions/0019-platform-rule-unification.md).

# ADR-0019: Единая модель Platform Rule (dashboard + bindings)

Статус: **Принято** (фазы 0–2 реализованы; фаза 3 — deprecation legacy mini-DSL)
...
- Один язык (CEL), один engine, одна документация ([BINDINGS](../bindings.md), [platform-logic](../platform-logic.md)).
...
| Фаза | Содержание |
|------|------------|
| 0 | ADR, [platform-logic](../platform-logic.md), обновление BINDINGS/DASHBOARDS |
