...
High-rate MQTT telemetry выявила узкие места (см. [0017-telemetry-ingest-pipeline](0017-telemetry-ingest-pipeline.md)):
...
Весь ingress и automation остаётся в `ispf-server`. Horizontal scale = **N stateless JVM replicas**, sharing one PostgreSQL (+ optional Redis/NATS), или larger dedicated host с tuned thread pools — не отдельный ingress worker process. См. [0028-horizontal-active-active-cluster](0028-horizontal-active-active-cluster.md).
