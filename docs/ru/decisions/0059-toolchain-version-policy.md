...
| `netty-bom` **4.2.17/4.2.18.Final** (`enforcedPlatform`) | `ispf-driver-opcua(-server)`, `ispf-driver-mqtt`, `ispf-driver-sparkplug-b`, `ispf-driver-graph-db`, тесты `ispf-server` | Milo 0.6.x и Moquette 0.17 объявляют Netty 4.1.x, neo4j-bolt-connection-netty — 4.2.15 (CVE-2026-75595); bom поднимает весь граф на патченную линию 4.2 (поток CVE от Dependabot) | релизы Milo / Moquette / Neo4j сами объявят Netty ≥ 4.2.18; тогда убрать bom или свести к одной версии | drivers/OPC UA | ежеквартально |
...
| `tomcat.version` **11.0.26** (`extra`) | `ispf-server`, `ispf-ai-agent` | Boot 4.1.1 BOM управляет Tomcat 11.0.24 — CVE-2026-65182 / 65905 / 68525 (ночной гейт Trivy) | Spring Boot BOM перейдёт на Tomcat ≥ 11.0.26 | platform/core | каждый patch Spring Boot |
| `bcprov/bcpkix/bcutil-jdk18on` **1.86** (`force`) | корневой `build.gradle.kts` | Eclipse Milo тянет 1.78.1, hadoop-common — 1.82; CVE-2025-14813, CVE-2026-8763, CVE-2026-13506; крипто должно резолвиться в одну патченную линию | Milo и hadoop-common объявят BC ≥ 1.85 | drivers/OPC UA + platform/historian | ежеквартально |
| `at.yawk.lz4:lz4-java` **1.11.3** (`force`) | корневой `build.gradle.kts` | kafka-clients 4.3.1 объявляет 1.10.2 (CVE-2026-59949) | kafka-clients объявит ≥ 1.11.3 | drivers/messaging | каждый bump Kafka |
| `commons-configuration2` **2.15.1** (`force`) | корневой `build.gradle.kts` | hadoop-common 3.5.0 объявляет 2.10.1 (CVE-2026-45205) | hadoop-common ≥ 3.5.1 объявит ≥ 2.15 | platform/historian | каждый bump hadoop |
...
| npm `overrides` (`fast-uri` 3.1.6, `browserslist` 4.28.7, `baseline-browser-mapping` 2.11.0, `uuid` 11.1.1, `brace-expansion` 1.1.18/2.1.4/5.0.12) | `apps/web-console/package.json` | поток GHSA Dependabot (maplibre — прямой bump; остальное транзитивно) | родители объявят патченные диапазоны | web-console | каждый проход Dependabot |
| npm `overrides` (`postcss` 8.5.23) | `tools/symbol-import`, `tools/symbol-pack-isa` | path traversal sourceMappingURL (GHSA-r28c-9q8g-f849 / GHSA-fxqj-rqcc-2cmp) | vite/tsx объявят postcss ≥ 8.5.23 | tooling | каждый проход Dependabot |
| npm `overrides` (`form-data` 4.0.6) | `tools/n8n-nodes-ispf` | n8n-workflow пинит form-data 4.0.0 (GHSA-fjxv-7rqg-78g4 / GHSA-hmw2-7cc7-3qxx) | n8n-workflow объявит form-data ≥ 4.0.6 | tooling | каждый bump n8n |
