...
Binding rules ([ADR-0010](0010-binding-rules-only.md)) остаются механизмом **реактивной логики на объекте**. Аналитические расчёты с окнами historian, DAG зависимостей и backfill — **отдельный движок**, пишущий результат обратно в дерево объектов (и NATS live sync в кластере).
...
Haystack / Brick — semantic overlay ([ADR-0021](0021-haystack-semantic-overlay.md)); analytics metadata добавляется поверх.
...
- Выход: `setVariableValue` + NATS fan-out ([ADR-0029](0029-cluster-live-variable-replica-sync.md)).
...
Расширение [ADR-0032](0032-replica-profiles-and-capabilities.md):
...
Детали приёмки: [analytics-platform-roadmap.md](../analytics-platform-roadmap.md).
...
- [ADR-0035](0035-historian-dual-write.md), [ADR-0032](0032-replica-profiles-and-capabilities.md)
- [ADR-0041](0041-multi-tag-historian-computations.md), [analytics-historian-cookbook.md](../../analytics-historian-cookbook.md)
