...
| 2 | Устройства | `root.platform.devices.{project}/*` | `instantiate_instance_type`, `apply_mixin_blueprint`, `create_virtual_device` |
...
**Три вида blueprints:** `get_automation_schema topic=instanceTypes` — MIXIN (примесь существующего объекта), INSTANCE (новый объект), SINGLETON (live-хаб в `singleton-blueprints.*` с логикой приложения).
