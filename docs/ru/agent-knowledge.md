...
1. **Бизнес-логика — в дереве объектов механизмов**, не на Java `ispf-server` и не на платформе React.
2. **Среда выполнения один:** вызов, рабочий процесс, оповещения, дашборды, привязки — через **дерево объектов** и API платформы.
...
Запрещено: платформа Flyway для приложений-таблиц, жестко запрограммированные маршруты BFF, отраслевая Java на сервере.
...
| **Б** | **Консоль администратора (ручная сборка)** | Инженер без пакета, итеративный HMI | Пользовательский интерфейс: модели, построитель информационной панели, инспектор | Панель приложений оператора |
...
| **Ч** | **Коммерческий пакет** | Лицензируемое решение | подписанный пакет + лицензионные ворота | как в C |

###Деревянные решения (кратко)
...
| Поиск по тегам Haystack | HaystackBindDialog (панель мониторинга), HaystackMetadataPanel (устройство) |
...
**Суть:** один манифест JSON/ZIP описывает всё решение. **Полярная звезда** для производства.
...
1. AI Studio → «Пакет бандл» или цепочка инструментов агента
...
| Бутстрап `tank-farm-demo` | нефтебаза-демо | Анонимная имитация SCADA нефтебазы (светильники) |
| Бутстрап `pipeline-scada` | трубопровод-Scada | Экранные формы РД-029 (15 мнемосхем, светильников) |
...
| Таблица HMI | Виджет информационной панели `object-table` + `selectionKey` | [DASHBOARDS.md](dashboards.md), [WIDGETS.md](widgets.md) |
...
| [APPLICATION_PRINCIPLES.md](application-principles.md) | принципы применения | Свод P1–P10, полярная звезда, антипаттерны |
...
| [PLATFORM_LOGIC.md](platform-logic.md) | логика платформы, правила | Правило платформы, контекст информационной панели |
...
| [SCADA.md](scada.md) | скада, имитация | Мнемосхемы, MIMIC, привязки, редактор (выравнивание, изменение размера, привязка) |
| [SCADA_MIMIC.md](scada-mimic.md) | scada-имитатор | диаграммаJson v2, имитирующая REST API |
...
| [DRIVERS.md](drivers.md) | водители | 58 драйверов, конфиг |
...
| [0004](decisions/0004-ai-artifact-generation-gates.md) | AI проверяет ворота |
| [0005](decisions/0005-tree-first-ai-agent.md) | Древо-первый агент |
| [0006](decisions/0006-mcp-agent-tool-adapter.md) | МКП |
| [0010](decisions/0010-binding-rules-only.md) | Обязательные правила |
| [0019](decisions/0019-platform-rule-unification.md) | Правило платформы/панель мониторинга |
...
3. зарегистрироваться (или включить в пакете) → миграции → функции → объекты/панели мониторинга.
