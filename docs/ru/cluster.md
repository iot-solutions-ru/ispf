...
| Правило | Зачем |
|---------|-------|
| **Один PostgreSQL** на все реплики | Одно дерево `root.platform.*`, driver locks, реестр кластера ([ADR-0028](decisions/0028-horizontal-active-active-cluster.md)) |
...
| **Разделение профилей** | `edge-api` в nginx; `io` / `analytics` / `compute` — внутренние ([ADR-0032](decisions/0032-replica-profiles-and-capabilities.md)) |
...
| worker-* | `compute` | нет | Потребитель `platform_jobs` (async reports) |
...
С рабочей станции (SSH-ключ — `ssh ispf-lab`, см. [lab-event-journal-stress.md](lab-event-journal-stress.md#ssh-с-рабочей-станции-один-раз)):
...
Ожидание для lab: **`nodesUp` = `nodesTotal` = 4**, профили `edge-api`, `edge-api`, `analytics`, `io`.
