...
| 3 | OT/IT drivers | 7.0 | **PARTIAL** | 47 `PRODUCTION` в `DriverProductionMatrix` (честность BL-191; партии A+B1+B2, включая sip, radius, ldap, jmx, nmea, message-stream, dhcp, ingress-syslog/snmp-trap/sflow); Milo OPC UA, j2mod, S7, BACnet4J — реальные; `opc-da` / `opc-bridge` / `ethernet-ip` честно **BETA** (оболочки / poll-only); DNP3 `writePoint` по-прежнему throws |
