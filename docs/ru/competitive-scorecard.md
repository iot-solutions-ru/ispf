...
| 3 | OT/IT drivers | 7.0 | **PARTIAL** | 55 `PRODUCTION` в `DriverProductionMatrix` (честность BL-191; партии A+B1+B2+B3, включая sip, radius, ldap, jmx, dhcp, ingress-*, iec104-server, omron-fins, mbus, smpp, xmpp, ipmi, wmi, odbc); Milo OPC UA, j2mod, S7, BACnet4J — реальные; `opc-da` / `opc-bridge` / `ethernet-ip` честно **BETA** (оболочки / poll-only); DNP3 `writePoint` по-прежнему throws |
