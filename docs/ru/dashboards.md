...
| `@dashboardContext` | **Планируется** — сеанс JSON (выборка, параметры, виджеты); см. [platform-logic](platform-logic.md) |
...
Пишите layout сразу в **84×8**. Миграции с 12×72 больше нет.

В `examples/*/generate_bundle.*` координаты тоже задаются в fine grid.

### Композиция экрана (для Builder и ИИ)

Дашборд должен читаться как операторская панель, не как случайная куча мелких карточек.

| Роль | Типичные `w`×`h` |
|------|------------------|
| KPI (value / indicator / gauge) | **21×14** или **28×14** (ряд из 3–4 плиток, сумма w = 84) |
| Chart / table / report | **≥42×28** |
| Nav chip / badge | **9–14×7** |
| SCADA mimic | **84×56…70** |

Правила: выравнивать ряды (один `y`, один `h`); заполнять ширину; размеры кратные 7; 4–10 виджетов на overview; KPI сверху → главный виджет → действия. Не использовать «наследие» `w=3,h=2` на 84-колоночной сетке — получится крошка.
...
Графики **net ↓ / net ↑** ссылаются на переменные объекты `ifInOctetsRate` / `ifOutOctetsRate` (Б/с). Они продолжаются привязка платформы `counterRate(ifInOctets)`/`counterRate(ifOutOctets)` в моделях `snmp-agent-v1` при каждом опросе SNMP; сырные Counter32 остаются в `ifInOctets` / `ifOutOctets`. Подробнее: [bindings](bindings.md).
...
Подробнее: [platform-logic](platform-logic.md), [bindings](bindings.md).
...
- Сетка: **84** fine-колонок, `rowHeight` **8**; позиция `x,y`, размер `w,h` в единицах сетки (например `w:21` ≈ четверть ширины, `h:14` ≈ две строки KPI)
...
Полный каталог с описанием каждого поля и примерами — **[widgets](widgets.md)**.
...
| `spreadsheet` | Таблица с формулами | [WIDGETS.md § электронная таблица](widgets.md) + [spreadsheet-widget](spreadsheet-widget.md) |
...
| `scada-mimic` | SCADA мнемосхема | [§ scada-mimic](widgets.md), [scada](scada.md) |
...
**Подробно:** [spreadsheet-widget](spreadsheet-widget.md). Операторская версия: [operator-guide](operator-guide.md).
...
![Конструктор дашбордов — графики, сетевой граф и spreadsheet](../assets/ispf-dashboard-builder.png)
...
- **Объединенные дашборды** (`root.platform.federation.*` или привязка к `DASHBOARD`): сохранение макета/заголовка проксируется на удаленном узле; пути виджетов в пользовательском интерфейсе локальными (см. [federation](federation.md))
