...
Пример правила информационной панели (планируется):
...
**Перекрестный объект:** активатор на удаленном пути + `refAt("path", var)` в выражении. При удаленной переменной (в т.ч. телеметрии водителя) `BindingPropagationListener` пересчитывает правила на потребительских объектах.
