...
`spreadsheet` — HMI-таблица с адресацией ячеек A1 (`A1`, `B2`, `C10`) и формулами на клиенте. Адаптация для сменных калькуляторов, сводок, простых расчетов и технологических таблиц без отдельного Excel.

Сводный каталог всех виджетов: [WIDGETS.md](widgets.md). Основы дашбордов, `selectionKey` и макет: [DASHBOARDS.md](dashboards.md).

## Почему может быть `#NAME?`

`#NAME?` означает, что формула движения не распознает имя функции. В ISPF это может быть:

- опечатка в имени функции: `=SUMM(A1:A10)` вместо `=SUM(A1:A10)`;
- После исправления стандартных функций вроде `SUM`, `AVERAGE`, `MIN`, `MAX`, `IF`, `ROUND` определяются **встроенным движком ISPF** (`ispfSheetEval`) вместе с `ISPREF`, `ISPSUM`, `ISPHIST`.

## Режимы

| Режим | Когда использовать | Поведение |
|-------|--------------------|-----------|
| `free` | Оператор сам выполняет таблицу | Любая неблокированная ячейка редактируется; строка формул, отмена/повтор, копирование/вставка, **импорт/экспорт XLSX**, CSV |
...
| Элемент | Описание |
|---------|----------|
| Формула Строка | Слева адрес выделенной ячейки, справа сырое правило: текст, число или формула с `=` |
| Сетка | Показывает вычисленные значения; формулы пересчитываются сразу |
| Панельные инструменты | В `free`: отмена/повтор, копирование/вставка, вставка/удаление строк и столбцов, **импорт XLSX**, **экспорт XLSX**, экспорт CSV |
| Вкладки листов | Несколько листов в одной книге; переключение вкладками; перекрестная формула (`=Sales!A1`) |
| Переплет-ячейки | Live-данные функции ISPF; не редактируется оператором |
...
Диапазоны пишутся как в Excel: `A1:A10`, `A1:C5`, **`D:D`** (весь столбец до конца сетки), **`Sheet!D:D`** (столбец на другом листе). Поддержите английские имена функций и русские псевдонимы (`СУММ` → `SUM`, `ЕСЛИ` → `IF`). Разделитель аргументов — `,` или `;` (как в русской локали Excel).
...
| Переплет-ячейки | Обновляют и отображаемое значение, и формулу кеш |
