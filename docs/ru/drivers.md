...
| `grpc` | `ispf-driver-grpc` | BETA | Apache-2.0 | gRPC: lab subset (преамбула HTTP/2), не protobuf RPC |
...
| `hitachi-hidic` | `ispf-driver-hitachi-hidic` | BETA | Apache-2.0 | Hitachi HIDIC: lab subset host-link |
...
| `lonworks` | `ispf-driver-lonworks` | BETA | Apache-2.0 | LonWorks: lab gateway, не LonTalk |
...
| `matter` | `ispf-driver-matter` | BETA | Apache-2.0 | Matter: lab gateway, не сессия CSA Matter |
...
| `opc-ae` | `ispf-driver-opc-ae` | BETA | Apache-2.0 | OPC A&E: lab subset, не DCOM |
...
| `opc-hda` | `ispf-driver-opc-hda` | BETA | Apache-2.0 | OPC HDA: lab subset, не DCOM |
| `opcua` | `ispf-driver-opcua` | PRODUCTION | Apache-2.0 | OPC UA клиент (Eclipse Milo): poll/subscribe/write/browse |
| `opcua-pubsub` | `ispf-driver-opcua-pubsub` | BETA | Apache-2.0 | OPC UA PubSub: lab subset заголовка UADP |
...
| `rockwell-csp` | `ispf-driver-rockwell-csp` | BETA | Apache-2.0 | Rockwell CSP: lab subset, не EtherNet/IP |
...
| `toshiba-t-series` | `ispf-driver-toshiba-t-series` | BETA | Apache-2.0 | Toshiba T-series: lab subset host-link |
...
| Каталог protocol stubs | только TCP reachability | Demand-driven codec в отдельном pack |
...
IEC 60870-5-104 **slave/server** (j60870). Конфиг: `listenPort` (2404), `commonAddress` (1). Устройства этого драйвера принимают подключения master; декодированные ASDU (single command, short-float/normalized setpoints, measured values) обновляют состояние IOA и уходят в ingress.

Маппинг точек: `<ioa>` (целое, например `2001`). Переменная: `value`, `quality` (`GOOD`/`NOT_CONNECTED`), `clientConnected`, `clientOriginatorAddress`.

**Write:** `writePoint` меняет серверное состояние IOA (число из `raw`/`value`) и синхронно обновляет переменную; ASDU подключённым master'ам не рассылаются.

Зрелость: **production** (`POLL` + `WRITE` + `QUALITY`). Loopback-тест: `Iec104ServerDeviceDriverTest` (j60870 client end-to-end). Ограничения: любой принятый TCP-коннект ставит `clientConnected=true` (до 104-handshake); `clientOriginatorAddress` не заполняется из ASDU (всегда 0); `writePoint` без проверки подключения.
...
M-Bus meter read (jMBus 3.x). Конфиг: `connectionType` (`tcp`/`serial`), `host`, `port` (10001) или `serialPort` (2400 baud).

Маппинг точек: `primary:secondary:register` (например `1:12345678:energy`); ненулевой secondary address переключает на broadcast primary `0xFD`. Поиск регистра: пара DIB/VIB, имя jMBus `Description` или `FunctionField`; если не найден — первая data record. Переменная: `value` (сырое значение `DataRecord` — коэффициенты масштабирования **не** применяются), `register`, `unit`.

Зрелость: **production**. Loopback-тест: `MbusDeviceDriverTest` (фейковый M-Bus TCP-счётчик, кадры RSP_UD). Только чтение. Ограничения: serial-путь не покрыт тестами; один синхронный round-trip к счётчику на точку.

### omron-fins (`ispf-driver-omron-fins`)

Omron FINS/TCP read. Конфиг: `host`, `port` (9600), `destNode`, `srcNode`.

Маппинг точек: `area:address:count` — области `CIO`, `WR`, `HR`, `AR`, `DM` (например `DM:100:1`). Переменная: `value` (16-битные слова big-endian через запятую), `memoryArea`, `address`, `count`.

Зрелость: **production**. Loopback-тест: `OmronFinsDeviceDriverTest` (фейковый FINS/TCP-сервер: handshake + чтение памяти). Только чтение. Ограничения: `connect()` ленивый (без handshake); новый TCP-сокет на точку за poll.
...
XMPP client probe (Smack 4.4). Конфиг: `host`, `port` (5222), `username`, `password`, `domain`, `securityMode`.

Маппинг точек: `presence` (connected + authenticated) или `rosterCount` (число записей roster). Переменная: `value`, `connected`, `authenticated`.

Зрелость: **production**. Loopback-тест: `XmppDeviceDriverTest` (встроенный в тест XMPP-сервер: stream negotiation + SCRAM-SHA-1 + ping end-to-end). Ограничения: `SecurityMode.disabled` — **без TLS**; в рантайме нужны модули `smack-xmlparser-xpp3` и `smack-java8` (переведены в `implementation` — без них Smack падает с `ExceptionInInitializerError`).

### ipmi (`ispf-driver-ipmi`)

IPMI 2.0 по RMCP+ (Verax IPMI client). Конфиг: `host`, `port` (623), `username`, `password`, `timeoutMs`.

Маппинг точек: `power` (статус питания шасси) или `sensor:<name>` (`Get Sensor Reading` — реальное чтение сенсора; `value` — сырое значение). Переменная: `value`, `reachable`, `raw`.

Зрелость: **production**. Loopback-тест: `IpmiDeviceDriverTest` (RMCP ping-путь + codec seam). Только чтение. Ограничения: полная аутентифицированная RMCP+-сессия end-to-end loopback-тестом не покрыта (вместо этого тестируется codec seam); RMCP ping используется как fallback достижимости.

### wmi (`ispf-driver-wmi`)

WMI-запрос через локальный вызов PowerShell/CIM — **только Windows**. Конфиг: `namespace` (`root\cimv2`), `query`, `timeoutMs`.

Маппинг точек: `WQL[:property]` — например `SELECT FreePhysicalMemory FROM Win32_OperatingSystem:FreePhysicalMemory`. Переменная: `value`, `supported` (false не на Windows), `status`.

Зрелость: **production**. Loopback-тест: `WmiDeviceDriverTest` (happy-path тесты запускаются только на Windows; на остальных ОС проверяется `supported=false`). Только чтение. Известная проблема: кодировка консоли на неанглийских локалях может искажать кириллицу (mojibake) — косметика, значения разбираются.

### odbc (`ispf-driver-odbc`)

Generic ODBC read через JDBC-мост. **Нужен внешний ODBC-JDBC bridge JAR** — в JDK его нет с Java 8; без него драйвер возвращает `NOT_AVAILABLE`. Конфиг: `url`, `username`, `password`, `query`, `timeoutMs`.
