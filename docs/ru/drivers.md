...
> **Честность (code audit / scorecard):** registry **PRODUCTION** ≠ ready-for-field. Известные пробелы, исторически помеченные PRODUCTION в матрице/доках: `opc-da` / `opc-bridge` (connectivity shell + parser tests), `dnp3` (**read/poll**; `writePoint` не реализован). Для field pilot считайте их **BETA**, пока не пройдут ready-for-field через [driver-promotion](driver-promotion.md). См. OT-измерение [competitive-scorecard](competitive-scorecard.md).
...
| `dnp3` | PRODUCTION (registry) | **Только poll/read** — write не реализован; field = BETA |
| `opc-da`, `opc-bridge` | PRODUCTION (registry) | **Shell / mapping tests** — не полный DA-стек; field = BETA |
...
**Security (v0.2):** для production — Sign/SignAndEncrypt с клиентским сертификатом и trust store. Текущий драйвер подключается только с **SecurityPolicy None** (lab/loopback).
...
Maturity: **production** в registry для Class 0/1/2/3 **poll/read** (loopback `Dnp3DeviceDriverTest`, BL-140). **`writePoint` не реализован** — не планируйте control/write field pilots на DNP3, пока write не появится; для сценариев записи считайте **BETA**.
