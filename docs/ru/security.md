...
Интерактивные пути доступа к переменным выполняются в режиме `VariableAclRequestContext.MEMBER` и проверяют `readRoles` / `writeRoles` через `VariableMemberAccessService`. Это относится к прямому чтению, записи, истории и экспорту, семантическому export/query Haystack/Brick, запросам, экспорту и выражениям аналитики, инструментам истории и аналитики AI-агента, доставке по WebSocket, редактору объектов, вычислению выражений и локальному прокси федеративного туннеля при выполнении от имени пользователя (`onBehalfOf*`). У событий и функций опционально задаётся `invokeRoles` (пусто = только object INVOKE); в веб-консоли доступны редакторы ACL переменной и `invokeRoles` (**BL-154 Готово**).

Фоновые планировщики и материализаторы остаются в режиме `SYSTEM`: ACL конечного пользователя к ним не применяется. Проверки состояния туннеля могут не передавать `onBehalfOf*`, но не могут возвращать значения переменных или историю.
