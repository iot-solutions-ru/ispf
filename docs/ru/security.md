...
![Ветка Security в дереве объектов](../assets/ispf-security.png)
...
Интерактивные пути доступа к переменным выполняются в режиме `VariableAclRequestContext.MEMBER` и проверяют `readRoles` / `writeRoles` через `VariableMemberAccessService`. Это относится к прямому чтению, записи, истории и экспорту, семантическому export/query Haystack/Brick, запросам, экспорту и выражениям аналитики, инструментам истории и аналитики AI-агента, доставке по WebSocket, редактору объектов, вычислению выражений и федеративным прокси-запросам от имени пользователя. У событий и функций опционально задаётся `invokeRoles` (пусто = только object INVOKE); в веб-консоли доступны редакторы ACL переменной и `invokeRoles` (**BL-154 Готово**).

Для федеративных HTTP-пиров аутентификация канала использует `authToken` пира (статический Bearer или Bearer сервисной учётной записи), а при отсутствии токена пира — перенаправленный Bearer вызывающего пользователя. Хаб также передаёт `X-ISPF-On-Behalf-Of-User`, `X-ISPF-On-Behalf-Of-Roles` со списком ролей через запятую и `X-ISPF-On-Behalf-Of-Tenant`. На пире `FederationOnBehalfOfFilter` устанавливает делегированный principal только с пересечением ролей аутентифицированного канала и ролей, заявленных в on-behalf-of заголовке, после чего применяется режим `MEMBER`. Поэтому учётная запись канала задаёт верхнюю границу полномочий, а не передаёт конечному пользователю свои привилегии. Для прокси-запросов через туннель сохраняется установка principal доверенного канала, так как канал WebSocket уже аутентифицирован.

Фоновые планировщики и материализаторы остаются в режиме `SYSTEM`: ACL конечного пользователя к ним не применяется. Проверки состояния федеративных пиров аутентифицируют только канал и не передают on-behalf-of identity; они не могут возвращать значения переменных или историю.
