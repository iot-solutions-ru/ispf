...
- Приложение оператора MoM доступно только для чтения для конфигурации; второй пилот агента доступен только для чтения ([BL-179](roadmap.md)).

См. также: [OPERATOR_GUIDE.md](operator-guide.md), [deploy/docker-compose.edge-arm.yml](../deploy/docker-compose.edge-arm.yml), [roadmap.md](roadmap.md) BL-188.
