...
Полная концепция — [FEDERATION.md](federation.md), дорожная карта [ROADMAP.md § Phase 4–8](roadmap.md).
...
**Bind** («заражение»): существующий или новый локальный узел получает переменные `federationProxy`, `federationPeerId`, `federationRemotePath`. Чтение/запись/история/панель мониторинга проксируются через `FederationProxyService`. Локальный драйвер (DEVICE) останавливается при привязке; локальные переменные определяются, но их привязку скрыты. Перед наложением сохраняется снимок (`displayName`, `description`, `type`, флаг работающего драйвера); **отменить** состояние этих свойств и перезапускает водителя, если он работал до привязки.
...
Тесты с пометкой `@Isolated` (`FederationChaosIntegrationTest`, `FederationTunnelIntegrationTest`, `FederationStoreForwardIntegrationTest`) — отсутствие параллельного выполнения с другими ИТ-классами. Ночные ворота: [ci-nightly.yml](../.github/workflows/ci-nightly.yml) задание **Ворота интеграции федерации (S27)**. При сбое тайм-аута сортировка выполняется согласно [CI_FLAKY_TRIAGE.md](ci-flaky-triage.md) (P1, если &lt;1×/неделя).
...
Федеративная информационная панель (`federationProxy=true` под `root.platform.federation.*` или привязка к `DASHBOARD`) раньше возвращала **403** к `PUT layout/title`. Теперь:
...
| объект-таблица | Строки из поддерева объединенных устройств; SelectionKey детализирует информационную панель сайта |
...
| Второй пилот агента | Охватывается только префиксами приложений оператора концентратора ([OPERATOR_GUIDE.md](operator-guide.md)) |
...
- Приложение оператора MoM доступно только для чтения для конфигурации; второй пилот агента доступен только для чтения ([BL-179](roadmap-phase-25.md)).

См. также: [OPERATOR_GUIDE.md](operator-guide.md), [deploy/docker-compose.edge-arm.yml](../deploy/docker-compose.edge-arm.yml), [ROADMAP_PHASE25.md](roadmap-phase-25.md) BL-188.
...
- ~~Область арендатора в API федерации~~ — **Выполнено v0.3.0** (`FederationAccessService`, одноранговый CRUD только для администратора, область действия прокси).
