...
**Полярная звезда:** ИИ не пишет Java/React в `main`; только проверенные артефакты (пакеты, модели, дашборды, функции, события) и узлы дерева с помощью инструментов платформы.
...
При последующих поворотах (по умолчанию): статические драйверы/примеры/функции опущены; **живой снимок** все еще обновлен.
...
Петля ReAct на платформе с жестким подножком (по умолчанию 96, `ispf.ai.agent-max-steps`). Агент работает до тех пор, пока не закончит работу или не достигнет ограничения — **без пауз в середине выполнения**. Кооператив **отменить** во время бега (`POST .../cancel`); живые шаги через `GET .../progress` (опросы пользовательского интерфейса ~1 с). Только для администратора.

**Многоходовые сеансы** (по типу курсора): каждый чат представляет собой сеанс в памяти на стороне сервера, история ходов которого воспроизводится в LLM (компактные сводные данные о пользователях/помощниках, без необработанного инструмента в формате JSON). `AgentRunState` (подтвердить → импортировать шлюзы) сохраняется на протяжении всего чата.
...
| `search_context` петля | `AgentLoopGuard` вводит подсказки о остановке после 3 повторений; инструменты информационной панели, документированные в строке |
| Достигнут предел шага | Поворот заканчивается на `OK` + `stepLimitReached` + предложение «Продолжить» (`agent-max-steps`, по умолчанию 256) |
...
|-----------|-----------------|------|
| HTTP JSON-RPC | `POST /api/v1/ai/mcp` | То же, что и API агента (носитель администратора/заголовок локальной роли) |
