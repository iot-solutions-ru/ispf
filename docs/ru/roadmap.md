...
| БЛ-154 | **ACL для каждой переменной** | П2 | **Готово** — R/W + history + `invokeRoles` API/UI. Follow-up по доверенному каналу закрыт: member ACL действует для аналитики, федеративного туннеля с `onBehalfOf`, инструментов агента и WebSocket |
