...
| БЛ-154 | **ACL для каждой переменной** | П2 | **Готово** — R/W + history + `invokeRoles` API/UI. Follow-up по доверенному каналу закрыт: member ACL для аналитики, federation tunnel + HTTP peer on-behalf-of (channel∩roles), агента, WebSocket, Haystack/Brick; hub fail-closed для remote-only proxy vars |
