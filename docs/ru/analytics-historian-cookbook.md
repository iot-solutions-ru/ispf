...
| CEL composite | `(avg(root.../a/temperature, 5m) + avg(root.../b/temperature, 5m)) / 2.0` | `cel` |
...
| Новый aggregate-примитив | Tier C — Java SPI pack | [§ Tier C](analytics-formulas-and-packs.md#tier-c--пакеты-расширений-java-spi) |

Составляйте выражения из `avg(ref, window)` / `live(ref)`, сохраняйте как Tier B или поставляйте Tier C pack.
...
**UI:** … встроенные функции (`avg`, `live`, …) — в каталоге редактора.
...
1. **analytics-chain-a:** `avg(root.platform.devices.demo-sensor-01/temperature, 1h)` → `derived-a`
...
**Важно:** `@historianRuleMeta` — **не источник данных** для выражений. Источник — PlatformRef на переменные (`root.../temperature`, `root.../derived-a`, …).

---

## Рецепт 5 — полный пример на prod: `analytics-demo`

Развёрнутый эталон на ${ISPF_BASE_URL:-https://ispf.example.invalid} (скрипты в репозитории). Демонстрирует цепочку ADR-0041 + дашборд + multi-tag query.
...
CEL: `avg(root.platform.devices.analytics-demo.chain-a/derived-a, 5m)`.
...
| Путь тега `objectPath/tag/ruleId` | ✅ | Каталог, lineage, DAG |
...
| GET | `/api/v1/platform/analytics/tags/by-path?path=` | Один тег (`objectPath/tag/ruleId`) |
