...
- Java-функции: при `bootRun` в `javac` попадает манифестный `Class-Path` единственного classpath-jar; ответ invoke приводится к `outputSchema`, как у скрипта.
- Удаление `DEVICE` останавливает драйверы под путём (включая потомков); сиротский `stop` / poll больше не требуют узел в дереве.
- BPMN-таймеры (boundary и intermediate catch) срабатывают сами через leader-locked `WorkflowDueTimerScheduler` (`ispf.workflow.timer-poll-ms`, по умолчанию 2 с); ручной endpoint остаётся.
