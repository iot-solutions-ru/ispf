...
- `queryScalar` / `queryRows` со спецификацией Object Query в одинарных кавычках: `resolveObjectQuerySpec` принимал строку за путь переменной, если внутри JSON был `/` (часто в `"path":"…"`), и падал как неверная ссылка. Кавычки снимаются раньше, чем выбор «ссылка или JSON».
