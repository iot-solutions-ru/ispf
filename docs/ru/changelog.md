...
- Валидатор скрипта функции отвергал `queryRows`, `scan_objects`, `for_each_row` и `apply_query_patch`, хотя исполнитель их уже выполнял — сохранение через валидатор падало с «Unknown script step type». Эти четыре типа теперь в списке известных шагов и проверяются как у исполнителя.
