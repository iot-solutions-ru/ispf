...
- Вход invoke: клиентская схема с полями подменяла `inputSchema` дескриптора, пустое тело становилось `DataRecord.empty` без проверки обязательных полей. Теперь строки проецируются на `inputSchema` (лишнее отбрасывается, типы приводятся, нет обязательного поля — ошибка); схема клиента контракт не заменяет.
