...
- BPMN-таймеры (boundary и intermediate catch) регистрировали срок, но сами не срабатывали: продолжение было только через `POST .../instances/{id}/timer`. Добавлен leader-locked `WorkflowDueTimerScheduler` (`ispf.workflow.timer-poll-ms`, по умолчанию 2 с). Ручной endpoint остаётся для проверки и принудительного срабатывания.
- Колонка запроса с историком при сбое чтения становилась пустой ячейкой: запрос выглядел успешным, будто выборок не было. Отказ ACL для члена по-прежнему пропускает поле; любой другой сбой историка теперь валит запрос.
- Сводка запроса по объектам считала пустую ячейку и нечисло нулём: минимум столбца с пропуском становился нулём, среднее делилось на все строки. Пустая ячейка в сводке не участвует; настоящий ноль остаётся нулём; нечисло — ошибка запроса. Минимум, максимум и среднее по пустому числовому набору больше не притворяются нулём.
