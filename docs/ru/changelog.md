...
- Поля пути объекта: там, где путь уже существующего объекта вводили текстом, стоит выбор из дерева. Это Apply mixin и родитель Instantiate (кнопка «взять выбранный объект»), расписание, цель и триггер SQL-привязки, расписание и источник данных при создании объекта, пути шагов скрипта, BPMN `objectPath` / `workflowPath` / `targetObject`, цель программы процесса, путь действия mimic, пути виджетов и префикс папки в строке алармов. Путь по-прежнему можно вставить с клавиатуры. Выражения, JSON драйвера и путь только для чтения остаются текстом. Удалённый путь federation тоже текст: объект лежит на peer, а не в локальном дереве.

### Исправлено

- Вход invoke: клиентская схема с полями подменяла `inputSchema` дескриптора, и во вход функции попадали лишние поля и типы клиента. Теперь каждая строка проецируется на `inputSchema` (лишнее отбрасывается, типы приводятся, пропущенное значение заполняется как при приведении в скрипте). События, запись драйвера и federation по-прежнему принимают схему клиента.
- Поле `INTEGER`: правило и выход скрипта усекали дробь через `intValue()` (`1.9` → `1`), значения вне диапазона `int` молча оборачивались; `DataRecord` принимал только `Integer` и отвергал `Long` в диапазоне. Общий `IntegerValues.requireInt` принимает целое в диапазоне `int` (включая `Long`) и отвергает дробь и выход за диапазон на всех трёх путях.
- Ошибка выражения правила маскировалась под пустой результат: `BindingExpressionEvaluator` глотал `ExpressionException`, движок писал только «Expression returned empty», цель не менялась. Теперь невычислимое выражение рвёт пересчёт с текстом выражения и сообщением движка; пустое тело по-прежнему значит «нечего считать».
- Правила привязки показывают сбой: упавший `call()` / `queryScalar()` / `queryRows()` / `fire()`, невычислимое условие или обрыв цепочки по лимиту проходов/глубины выглядели успехом или честной ложью. Теперь это ошибка пересчёта с путём и причиной; вложенный `call()` во время уже идущего вызова запрещён; посчитанная ложь и пустое условие без изменений.
- Запросы по объектам: пустая/нечисловая ячейка в сводке больше не считается нулём, сбой колонки историка больше не маскируется пустой ячейкой. Пустые пропускаются (настоящий `0` остаётся), нечисло и ошибка историка валят запрос (отказ ACL члена по-прежнему пропускает поле); `min` / `max` / `avg` без чисел — ошибка, а не фейковый `0`. `queryScalar` / `queryRows` больше не принимают JSON в кавычках за путь переменной из‑за `/` внутри `"path":"…"` — кавычки снимаются до выбора ref vs JSON.
- Java-функции: при `bootRun` в `javac` попадает манифестный `Class-Path` единственного classpath-jar; ответ invoke приводится к `outputSchema`, как у скрипта. `FunctionScriptValidator` принимает шаги Object Query, которые исполнитель уже умел (`queryRows`, `scan_objects`, `for_each_row`, `apply_query_patch`), вместо «Unknown script step type» при сохранении.
- Удаление `DEVICE` останавливает драйверы под путём (включая потомков); сиротский `stop` / poll больше не требуют узел в дереве.
- BPMN-таймеры (boundary и intermediate catch) срабатывают сами через leader-locked `WorkflowDueTimerScheduler` (`ispf.workflow.timer-poll-ms`, по умолчанию 2 с); ручной endpoint остаётся. После прерывающего boundary timer (`cancelActivity=true`) при сохранении инстанса закрываются «осиротевшие» `OPEN`/`CLAIMED` строки work-queue, чей узел больше не pending. Сбой `ispf:function` при complete user task теперь отклоняет complete через `WorkflowException`, а не только логируется с закрытием задачи.
- Поле `DOUBLE` принимает любое конечное число как дробь (в т.ч. JSON-целые больше 32 бит).
