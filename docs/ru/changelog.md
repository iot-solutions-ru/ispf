...
- Функция на user task при complete: сбой вызова только логировался, задача всё равно закрывалась. Теперь `WorkflowException` отклоняет complete; задача остаётся открытой.
