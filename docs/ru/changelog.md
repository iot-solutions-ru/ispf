...
- После прерывающего boundary timer (`cancelActivity=true`) задача в work-queue оставалась `OPEN`/`CLAIMED`, хотя инстанс уже ушёл с user task. При сохранении инстанса теперь закрываются «осиротевшие» открытые/взятые строки очереди, чей узел больше не pending.
