...
**Решение проекта:** 0001, 0002 (app/platform boundary, dogfooding через reference apps вне `main`).
...
Критерий нового REQ-PF: *можно ли выразить потребность через существующий или обобщённый механизм дерева объектов?* Если да — расширяем механизм; если нет — gate 0002.
...
Развитие platform идёт через **dogfooding** с обязательным gate (0002):
...
| **FW-40**  | AI Layer: `LlmProvider` SPI        | **Done** | AI    | P3+ |
| **FW-41**  | ContextPack для LLM                | **Done** | AI    | P3+ |
...
| **FW-60**  | Time & timezone contract           | Partial | TIME  | P2  |
...
Post-PF (P2–P3+) — REQ-FW (§12)
  ...
  Sprint H (P2–P3) — time & timezones ([Phase 21](ROADMAP.md#phase-21--time--timezones))
    FW-60 / BL-66…71 — ADR-0020, user/device TZ, historian observedAt
```
...
Порядок внедрения уточняется по потребностям app-команд (gate 0002). Типичные кластеры:
...
| Time | **TIME** | Store UTC, display local; user/device TZ | FW-60 |

### 12.2 REQ-FW-01 — ADR `docs/decisions/`

**Зачем:** архитектурные решения в отдельных ADR с контекстом, альтернативами и последствиями; сейчас ISPF ссылается на 0001/0009 без файлов.
...
- [x] ADR: bundle = tree packaging (**0014**), federation topology (**0015**), TimescaleDB retention (**0016**).
...
| ID | Компонент | Задачи |
| -- | --------- | ------ |
| **FW-40** | `LlmProvider` SPI | [x] `ispf-ai-api` + openai-compatible/ollama adapters; Spring profile config |
| **FW-41** | ContextPack | [x] `ai/context/`, `tools/ai-pack/build.py`, classpath pack |
| **FW-42** | ToolRegistry | [x] validate/dry-run/list models; `ai_tool_audit`; REST `/api/v1/ai/**` |
| **FW-43** | Platform Studio | [x] Web Console AI Studio tab; publish = package import |

**Acceptance FW-42:** [x] prompt → generated bundle JSON → `validate` tool → CI green → deploy на local (see `AiToolApiTest`, `BundleManifestValidatorTest`).

**Документация:** [AI_DEVELOPMENT.md](AI_DEVELOPMENT.md), [0004](decisions/0004-ai-artifact-generation-gates.md), [0005](decisions/0005-tree-first-ai-agent.md).

**Принцип:** ИИ не пишет произвольный React/Java в `main`; только артефакты + gates.

#### FW-44 — Tree-first agent (v0.7.5)

- [x] `TreeFirstAgentService` + platform tools (`PlatformAgentToolRegistry`)
- [x] Multi-turn in-memory sessions + Web Console chat UI
...
- [x] MCP adapter ([0006](decisions/0006-mcp-agent-tool-adapter.md))
...
### 12.9 REQ-FW-60 — Time & timezone contract

**Зачем:** операторы, площадки и устройства могут находиться в разных IANA timezones. Без явного контракта журналы, графики «за сутки», сменные отчёты и метки событий теряют смысл. Сегодня платформа хранит UTC, но UI использует browser default, historian пишет ingestion time, у user/device нет поля timezone.

**Решение (ADR):** [0020](decisions/0020-time-and-timezones.md) — store UTC, display local; IANA zones only; dual timestamps `observedAt` / `ingestedAt` для телеметрии; optional `occurredAt` для events.

**Задачи (волна BL-66…71, [CODE_AUDIT_BACKLOG.md § Wave H](CODE_AUDIT_BACKLOG.md#wave-h--time--timezones)):**

- [x] BL-66: ADR-0020 Accepted + inventory hot paths
- [x] BL-67: User `timeZone` в `platform_users` + profile API + `TimezoneSwitcher` + `formatDateTime` в Web Console
- [x] BL-68: Device `timeZone` metadata + inheritance от folder/site
- [x] BL-69: Historian `observedAt` + driver SPI (`updateVariable` overload, virtual/MQTT)
- [x] BL-70: Calendar-boundary history + reports (`calendarRange`, `reportTimeZone`)
- [x] BL-71: `POST /events/fire` optional `occurredAt` + skew guard

**Gate 0002:** второй сценарий — multi-site SCADA (MiniTec + устройство в другом TZ) + operator report «за смену» в user TZ.

**Acceptance:** ADR-0020; user TZ в profile; device TZ в object tree; API отдаёт UTC; UI display в user TZ; опционально `observedAt` для historian и `occurredAt` для events.

**Out of scope v1:** хранение local time в БД; auto-detect TZ по GPS/IP; Keycloak `timezone` claim sync (follow-up к BL-67).

---

### 12.10 Границы REQ-FW (out of scope)
...
### 12.11 Gap-registry (живой срез)

**Источник правды:** [GAP_REGISTRY.md](GAP_REGISTRY.md). Обновлять при закрытии REQ-FW / REQ-PF.

**Sprint planning:** [GAP_REGISTRY.md § Sprint planning](GAP_REGISTRY.md#sprint-planning) + §3 этого файла. Текущая волна — [ROADMAP.md § Phase 20](ROADMAP.md#phase-20--code-audit-backlog-ui-drivers-scale) ([CODE_AUDIT_BACKLOG.md](CODE_AUDIT_BACKLOG.md)) + [Phase 18.1](ROADMAP.md#phase-18--frontend-e2e--demand-driven-drivers) (Playwright) + [Phase 21](ROADMAP.md#phase-21--time--timezones) (FW-60 / BL-66…71).
...
| Дата       | Изменение                                                                   |
| ---------- | --------------------------------------------------------------------------- |
| 2026-06-30 | REQ-FW-60 time & timezones: ADR-0020, Wave H BL-66…71, ROADMAP Phase 21 |
| 2026-06-28 | Wave G Haystack/Brick: BL-56…62 в [CODE_AUDIT_BACKLOG.md](CODE_AUDIT_BACKLOG.md), ROADMAP 20.22 |
| 2026-06-28 | Code audit backlog: [CODE_AUDIT_BACKLOG.md](CODE_AUDIT_BACKLOG.md) BL-01…55, ROADMAP Phase 20 |
| 2026-06-28 | Phase 18 сужена: mini-TEC acceptance и v0.8.0 prod rollout сняты с roadmap |
| 2026-06-23 | Phase 18 kickoff: mini-TEC reference walkthrough, ROADMAP 18.1–18.6 |
| 2026-06-23 | Phase 17: post-baseline hardening; MCP ContextPack resources; gap-registry sprint planning; ADR 0017–0019 |
