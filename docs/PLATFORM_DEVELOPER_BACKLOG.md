...
| ---------- | ---------------------------------- | ----------- | ----- | --- |
| **FW-01**  | ADR `docs/decisions/`              | **Planned** | DOC   | P2  |
| **FW-02**  | Platform gap-registry              | **Planned** | DOC   | P2  |
| **FW-10**  | RSA licensing (commercial bundle)  | **Planned** | LIC   | P2  |
| **FW-11**  | `installationId` + LicenseBuilder  | **Planned** | LIC   | P2  |
| **FW-12**  | Bundle dependency manifest         | **Planned** | LIC   | P3  |
| **FW-20**  | MES reference walkthrough + demo   | **Planned** | REF   | P2  |
| **FW-30**  | Solution developer public API doc  | **Planned** | API   | P2  |
| **FW-31**  | Event catalog в bundle             | **Planned** | API   | P3  |
| **FW-32**  | Event bus vs sync RPC (контракт)   | **Planned** | NET   | P3  |
...
- [ ] Создать `docs/decisions/` и шаблон ADR (статус, контекст, решение, последствия).
- [ ] Перенести в файлы: app/platform boundary (**0008**), dogfooding gate (**0009**).
- [ ] ADR: bundle = tree packaging; federation topology; TimescaleDB retention; commercial vs Apache core.
- [ ] В `ARCHITECTURE.md`, `APPLICATIONS.md`, `PLUGINS.md` — ссылки на `docs/decisions/`, не inline-only.
...
- [ ] Поддерживать таблицу §12.10 (или вынести в `docs/GAP_REGISTRY.md` при росте объёма).
- [ ] Правило: закрытие REQ-PF / REQ-FW → обновление строки + [ROADMAP.md](ROADMAP.md) при необходимости.
- [ ] Сводная таблица «высокий / средний / низкий» приоритет.
...
- [ ] Формат `license.json`: plugin/bundle name, minVersion, expiry, content hash, RSA signature.
- [ ] Публичный ключ в platform (константа или config); **без** привязки ядра к closed-source.
- [ ] `POST .../deploy` и загрузка licensed driver → verify до apply; ошибка → 403 + audit log.

#### FW-11 — `installationId` + LicenseBuilder

- [ ] Файл `.ispf-installation-id` (hex) при первом prod start; сверка при последующих.
- [ ] CLI `tools/license-builder/` (или отдельный repo): генерация ключей, выпуск `license.json` по installationId + hash bundle.
- [ ] Документация для поставщика commercial bundle.

#### FW-12 — Bundle dependency manifest

- [ ] Поле `requires[]` в bundle manifest: appId / bundleId, minVersion.
- [ ] Deploy проверяет порядок и наличие зависимостей.
...
- [ ] `docs/REFERENCE_MES_WALKTHROUGH.md` (или расширение [LAB_TRAINING.md](LAB_TRAINING.md)): процессы, шаги, object paths, события, operator screens.
- [ ] `examples/mes-reference/` — bundle + synthetic demo seed (JSON/JSONL), smoke script.
- [ ] Привязка шагов к REQ-PF (deploy, correlator, workflow, BFF).
...
- [ ] Раздел `docs/SOLUTION_DEVELOPER_PUBLIC_API.md` (или § в [SOLUTION_DEVELOPER_GUIDE.md](SOLUTION_DEVELOPER_GUIDE.md)): разрешено / запрещено, stable REST paths, bundle schema version.
- [ ] Таблица контрактов: deploy API, object tree, BFF, drivers SPI — что считается semver-stable.

#### FW-31 — Event catalog в bundle

- [ ] Секция `events[]` в bundle: `id`, roles[], optional payload schema (JSON Schema).
- [ ] Загрузка в registry при deploy; WS subscribe проверяет роль по catalog.

#### FW-32 — Event bus vs sync RPC

- [ ] Документ `docs/MESSAGING.md`: NATS/event journal = async; function invoke / federation = sync; когда что использовать.
- [ ] Явные имена топиков/паттернов; без смешения в одном «универсальном» канале.
...
### 12.10 Gap-registry (живой срез, июнь 2026)

Обновлять при закрытии REQ-FW / REQ-PF. Источник правды по PF — §3; по FW — §12.2–12.8.

| Подсистема | Готовность | Главный пробел | REQ |
| ---------- | ---------- | -------------- | --- |
| REQ-PF baseline | ~100% | polish stub drivers | PF-14, §10 |
| ADR discipline | ~20% | нет `docs/decisions/` | FW-01 |
| Gap-registry process | ~40% | таблица §12.10 — старт; процесс FW-02 | FW-02 |
| Commercial licensing | ~10% | нет RSA verify | FW-10…12 |
| MES reference | ~50% | lab training есть, нет полного MES walkthrough | FW-20 |
| Solution public API doc | ~60% | размазано по guides | FW-30 |
| Event catalog (declarative) | 0% | только runtime events | FW-31 |
| Messaging contract doc | ~30% | NATS есть, контракт не формализован | FW-32 |
