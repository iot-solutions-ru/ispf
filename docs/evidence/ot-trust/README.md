...
| **Field trust** | **In progress** — Pilot #1 soak days **1–3** + C5; **Pilot #2 MQTT** day **1** + C5 on `lab-ot-vlan-192.168.100`. Not OT 10/10. |
...
**Field track:** Pilot #1 (Modbus) days **1–3** soak green + C5. **Pilot #2 (MQTT fleet)** day **1** + C5 green on same VLAN. Days remaining on both journals. Jump SSH key auth restored.
