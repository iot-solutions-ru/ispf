...
**Post-audit note (2026-08-30):** BL-154 trusted-channel follow-up closed on branch work (analytics/agent/WS/editor/Haystack/Brick + federation tunnel/HTTP on-behalf-of with role intersection; G-05 interactive residual closed). Does **not** re-score dimension 10 in the frozen 0.9.102 matrix — carry into the next full audit. Remaining Security gaps unchanged: optional hard schema routing, WebAuthn (BL-194), pen-test.
...
| 10 | Security / RBAC | 8.5 | **PARTIAL** | TOTP MFA GA **Done** (BL-153); per-var/event/function ACL **Done** (BL-154) including trusted-channel close (analytics/federation/agent/WS — 2026-08-30, pending full audit rescore); audit SIEM + role-template scopes **Done** (BL-156/157); SaaS tenant-admin + logical A≠B path/API **Done** (BL-155); hard schema table routing still optional; WebAuthn → BL-194 |
...
3. **AI (9.0 → 10):** BL-177…180 Done (harness + BL-178 52/52). The **2026-08-24 named-site oneshots are archived** under [`docs/evidence/ai-generator/`](../evidence/ai-generator/) from the **0.9.186 demostand** for HVAC, MES, and SCADA, with `softBudgetMet: true`. Residual for 10/10: re-soak on the current platform version (**0.9.190**) and, optionally, extend the named-site journal across multiple days. Keep the AI score frozen; do not infer new scores or live pass counts from these soft-evidence archives.
