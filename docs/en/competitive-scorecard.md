...
| 3 | OT/IT drivers | 7.0 | **PARTIAL** | 55 `PRODUCTION` in `DriverProductionMatrix` (BL-191 honesty; batches A+B1+B2+B3 incl. sip, radius, ldap, jmx, dhcp, ingress-*, iec104-server, omron-fins, mbus, smpp, xmpp, ipmi, wmi, odbc); Milo OPC UA, j2mod, S7, BACnet4J real; `opc-da` / `opc-bridge` / `ethernet-ip` honest **BETA** (shells / poll-only); DNP3 `writePoint` still throws |
