...
| 3 | OT/IT drivers | 7.0 | **PARTIAL** | 47 `PRODUCTION` in `DriverProductionMatrix` (BL-191 honesty; batches A+B1+B2 incl. sip, radius, ldap, jmx, nmea, message-stream, dhcp, ingress-syslog/snmp-trap/sflow); Milo OPC UA, j2mod, S7, BACnet4J real; `opc-da` / `opc-bridge` / `ethernet-ip` honest **BETA** (shells / poll-only); DNP3 `writePoint` still throws |
