...
| 3 | OT/IT drivers | 7.0 | **PARTIAL** | 36 `PRODUCTION` in `DriverProductionMatrix` (BL-191 honesty; batches A+B1: dnp3, haystack, kafka, coap, icmp, ip-host, telnet, modem-at, ssh, file, folder, application, imap, pop3, soap, web-transaction, http-server, jdbc, graph-db, jms); Milo OPC UA, j2mod, S7, BACnet4J real; `opc-da` / `opc-bridge` / `ethernet-ip` honest **BETA** (shells / poll-only); DNP3 `writePoint` still throws |
