...
| G-05 | **Per-variable / custom roles** | Variable roles + history ACL + event/function `invokeRoles` API/UI + ISA-95 template scopes on REST; interactive analytics, agent, WebSocket/editor, expression, and federation HTTP/tunnel on-behalf-of paths enforce `MEMBER` ACL | **Closed / Exists for interactive paths.** HTTP peer residual closed: the hub sends `X-ISPF-On-Behalf-Of-User`, `X-ISPF-On-Behalf-Of-Roles`, and `X-ISPF-On-Behalf-Of-Tenant`; the peer limits delegated roles to channel ∩ on-behalf-of roles, preventing service-account privilege escalation. Health probes remain channel-only with no variable values/history, and `SYSTEM` automation still does not apply end-user ACL | [BL-154](roadmap.md#phase-27--enterprise-security) **Done**, [BL-157](roadmap.md#phase-27--enterprise-security) **Done** |
