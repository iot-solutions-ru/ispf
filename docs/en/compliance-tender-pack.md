...
| G-05 | **Per-variable / custom roles** | Variable roles + history ACL + event/function `invokeRoles` API/UI + ISA-95 template scopes on REST; interactive analytics, agent, WebSocket/editor, expression, and federation tunnel on-behalf-of paths enforce `MEMBER` ACL | **Closed / Exists for interactive paths.** Residual: `SYSTEM` automation does not apply end-user ACL, and an optional HTTP peer `authToken` remains channel-trust for remote REST when the token identifies a service account | [BL-154](roadmap.md#phase-27--enterprise-security) **Done**, [BL-157](roadmap.md#phase-27--enterprise-security) **Done** |
