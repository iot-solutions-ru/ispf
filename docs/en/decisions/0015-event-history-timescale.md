...
Telemetry historian (`variable_samples`) already uses a TimescaleDB hypertable ([0009](0009-timescaledb-retention.md)). The event journal previously lived in a plain PostgreSQL table.
...
5. **PostgreSQL** remains for the relational core; column store (ClickHouse) — P3b ([0016](0016-clickhouse-event-journal.md)).

## Consequences

- Prod Docker: `timescale/timescaledb` image (already in [DEPLOYMENT.md](../deployment.md)).
...
- [OBJECT_MODEL.md](../object-model.md) — persistence tables
