...
| `AgentPlatformTurnGuard` | adapter | Finish with ERROR steps; empty dashboard/mimic; missing alert | 1–2 | **`AcceptanceVerdict`** wired to finish and judge. Check ids: `errors.none`, `device.telemetry`, `alert.configured`, `mimic.elements`, `dashboard.widgets`, `bundle.tests` | Verdict `PASS` required for execution finish | Keep the adapter; message lexicon for alert/mimic retires when the plan assignment type is the only trigger |
...
4. **Acceptance verdict** — **landed** as `AcceptanceVerdict`. Finish (`AgentPlatformTurnGuard`) and the pre-finish judge both require `PASS`. Checks: `errors.none`, `device.telemetry`, `alert.configured`, `mimic.elements`, `dashboard.widgets`, `bundle.tests`. `FAIL` is contradicted evidence; `PARTIAL` is a required check with no observation yet. A successful execution finish records `acceptance` on the step. Message lexicon for alert/mimic stays in this catalog until assignment type on the plan is the only trigger.
