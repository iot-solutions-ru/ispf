...
Сейчас backends: `jdbc` (PostgreSQL/Timescale, default) и `clickhouse` (opt-in). Для деплоев, где уже есть Cassandra или Scylla (CQL-совместимый кластер), нужен третий backend без дублирования hot path (`EventJournalAsyncWriter`, `VariableHistoryAsyncWriter`).

PostgreSQL остаётся для relational core (дерево объектов, workflow, RBAC).

## Решение

1. **Store value:** `cassandra` или `scylla` (алиас) — один и тот же Java backend (DataStax Java Driver 4.x).
2. **Конфиг:**
...
7. **Агрегации historian:** JVM bucket aggregation (как fallback в JDBC store), без server-side GROUP BY.
