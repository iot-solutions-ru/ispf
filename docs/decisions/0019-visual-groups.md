...
4. Structural children under `VISUAL_GROUP` are forbidden; membership changes use `PUT /api/v1/objects/by-path/group-members`.
5. On object delete, all groups are scanned and stale member paths removed.
6. Explorer multi-select uses composite row keys (`groupPath::memberPath`) for group references; bulk delete operates on canonical paths; remove-from-group is a separate contextual action.
