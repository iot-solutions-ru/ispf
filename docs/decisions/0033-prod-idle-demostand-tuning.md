...
Платформенные defaults ([ADR-0026](0026-elastic-telemetry-ingress.md)) рассчитаны на **throughput**: elastic L0–L3 пулы (ingress 4–32 workers, binding-async, object-change bus). На стендах с **малым** числом устройств (demo, HMI, edge gateway) те же defaults дают:
...
Остановка драйверов снижает нагрузку, но **не заменяет** выбор профиля: для demo нужны живые устройства при урезанных пулах и корректных publish modes.
...
| **Throughput** | ON (peak tuning) | benchmark | [LOAD_TESTING.md](../LOAD_TESTING.md) |
...
- [LOAD_TESTING.md](../LOAD_TESTING.md) — throughput
