...
Пока user task в `WAITING`, параллельно отсчитывается SLA. При срабатывании таймера выполняется ветка escalation (interrupting). Шаблон: [REFERENCE_ESCALATION_TEMPLATES.md](REFERENCE_ESCALATION_TEMPLATES.md).
...
Продолжает экземпляр, если deadline boundary timer или intermediate timer catch уже наступил.
