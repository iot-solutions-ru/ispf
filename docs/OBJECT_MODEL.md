...
**RELATIVE mixins** при create auto-apply’ятся только при **непустом** CEL (*Applicability condition*). Пустой CEL → только явный apply (`templateId`, API). Fixture-модели (`mqtt-sensor-v1`, …) не входят в core registry.
...
| `variable_samples` | История телеметрии (time-series сэмплы; Timescale hypertable — [0009](decisions/0009-timescaledb-retention.md)) |
| `event_history` | Журнал событий (Timescale hypertable — [0015](decisions/0015-event-history-timescale.md)) |
