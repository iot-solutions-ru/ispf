package com.ispf.driver.matter.codec;
...
/**
 * TCP session for the Matter/CHIP controller gateway lab (default port {@code 5540}).
 * <p>
 * Honesty: Matter/CHIP controller gateway lab — not full CSA Matter / CHIP SDK / Thread/BLE commissioning.
 * <p>
 * Dialect (newline JSON):
 * <pre>
 *   {"op":"get","point":"node:1:ep:1:cluster:OnOff:attr:OnOff"}{@code \\n}
 *   {"op":"set","point":"node:1:ep:1:cluster:OnOff:attr:OnOff","value":1}{@code \\n}
 *   {"op":"set","point":"node:1:cmd:On","value":1}{@code \\n}
 * </pre>
 * Responses: {@code {"ok":true,"value":...}} / {@code {"ok":false,"error":"..."}}.
 */
public final class MatterLabSession implements AutoCloseable {
    ...
    public MatterLabSession(String host, int port, int timeoutMs) throws IOException {
        socket = new Socket();
    ...
    private String transact(String jsonLine) throws IOException {
        ...
        if (line == null) {
            throw new EOFException("EOF from Matter controller gateway lab");
        }
        return line;
    }
    ...
    static double parseValue(String response) throws IOException {
        ...
        if (!matcher.find()) {
            throw new IOException("Matter controller gateway lab response missing value: " + response);
        }
    ...
    static void ensureOk(String response) throws IOException {
        ...
        if (ok.find() && "false".equalsIgnoreCase(ok.group(1))) {
            ...
            throw new IOException("Matter controller gateway lab rejected: " + message);
        }
        if (response.toLowerCase(Locale.ROOT).contains("\"error\"")) {
            Matcher err = ERROR_FIELD.matcher(response);
            if (err.find()) {
                throw new IOException("Matter controller gateway lab rejected: " + err.group(1));
            }
        }
    }
