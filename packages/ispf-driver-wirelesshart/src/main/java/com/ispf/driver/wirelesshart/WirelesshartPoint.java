 ...
 */
record WirelesshartPoint(int deviceAddress, int command) {
    ...
    static WirelesshartPoint parse(String mapping) throws DriverException {
        ...
        try {
            if ("pv".equals(normalized) || "primary".equals(normalized) || "primary-variable".equals(normalized)) {
                return new WirelesshartPoint(0, 1);
            }
            if (normalized.startsWith("cmd:")) {
                return new WirelesshartPoint(0, Integer.parseInt(normalized.substring(4).trim()));
            }
            if (normalized.startsWith("device:")) {
                String rest = normalized.substring(7).trim();
                int colon = rest.indexOf(':');
                if (colon < 0) {
                    return new WirelesshartPoint(Integer.parseInt(rest), 1);
                }
                int device = Integer.parseInt(rest.substring(0, colon).trim());
                String after = rest.substring(colon + 1).trim();
                if (after.startsWith("cmd:")) {
                    return new WirelesshartPoint(device, Integer.parseInt(after.substring(4).trim()));
                }
                return new WirelesshartPoint(device, Integer.parseInt(after));
            }
