...
/**
 * UDP session for the IEC 61850 Sampled Values subscribe/publish lab
 * (not IEC 61869 / 9-2LE full stack).
 * <p>
 * Datagram dialect (lab on UDP — default port 8503; 102 also accepted via config):
 * <pre>
 *   GET svID:MU1         →  VALUE 12.5
 *   SET sv:1:smp:0 7.0   →  OK
 * </pre>
 */
public final class Iec61850SvLabSession implements AutoCloseable {

    private final DatagramSocket socket;
    private final InetSocketAddress remote;
    private final int timeoutMs;

    public Iec61850SvLabSession(String host, int port, int timeoutMs) throws IOException {
    ...
    public void writeValue(String wireToken, double value) throws IOException {
        ...
        if (!response.trim().toUpperCase(Locale.ROOT).startsWith("OK")) {
            throw new IOException("IEC 61850 SV-lab SET rejected: " + response);
        }
    }
    ...
    static double parseValueResponse(String response) throws IOException {
        ...
        if (trimmed.isEmpty()) {
            throw new IOException("Empty IEC 61850 SV-lab response");
        }
        String upper = trimmed.toUpperCase(Locale.ROOT);
        if (upper.startsWith("ERR")) {
            throw new IOException("IEC 61850 SV-lab GET rejected: " + response);
        }
        ...
        try {
            ...
            throw new IOException("IEC 61850 SV-lab non-numeric value: " + response, e);
        }
    }
