...
/**
 * Email notification gateway — POSTs JSON {@code {to,subject,body}} to a configured HTTP relay.
 * Separate from generic {@code http} and from inbound {@code imap}/{@code pop3}.
 */
public class EmailDeviceDriver implements DeviceDriver {

    private static final DataSchema RESULT_SCHEMA = DataSchema.builder("emailResult")
    ...
    private static final DriverMetadata METADATA = new DriverMetadata(
            ...
            "0.1.0",
            "Sends email via HTTP relay (JSON to/subject/body). Configure relayUrl per device.",
            "ISPF",
            Map.of(
                    "relayUrl", "http://127.0.0.1:8090/email",
                    "timeoutMs", "15000",
                    "defaultTo", "",
                    "defaultSubject", ""
            ),
    ...
    private void applyConfig(String key, String value) {
        ...
        switch (key) {
            ...
            case "defaultSubject" -> defaultSubject = value.trim();
            default -> { }
        }
    }
    ...
    public void connect() throws DriverException {
        if (relayUrl == null || relayUrl.isBlank()) {
            throw new DriverException("email driver requires relayUrl");
        }
        if (!relayUrl.startsWith("http://") && !relayUrl.startsWith("https://")) {
            throw new DriverException("email relayUrl must be http(s): " + relayUrl);
        }
        client = HttpClient.newBuilder()
                .connectTimeout(Duration.ofMillis(timeoutMs))
                .build();
        connected = true;
        driverObject.log(DriverLogLevel.INFO, "Email relay ready (" + relayUrl + ")");
    }
    ...
    public void writePoint(String pointId, DataRecord value) throws DriverException {
        ...
        if (value == null || value.rowCount() == 0) {
            throw new DriverException("email write requires a non-empty DataRecord");
        }
        String mapping = lastMappings.get(pointId);
        if (mapping == null) {
            throw new DriverException("Unknown email point: " + pointId + " (poll once or map outbound)");
        }
        requireOutboundMapping(mapping);
        Map<String, Object> row = value.firstRow();
        String to = firstNonBlank(row, "to", "destination");
        if (to == null || to.isBlank()) {
            to = defaultTo;
        }
        String subject = firstNonBlank(row, "subject");
        if (subject == null || subject.isBlank()) {
            subject = defaultSubject;
        }
        String body = firstNonBlank(row, "body", "text", "message", "value");
        if (to == null || to.isBlank()) {
            throw new DriverException("email write requires to (or defaultTo)");
        }
        if (subject == null) {
            subject = "";
        }
    ...
    private DataRecord postJson(Map<String, Object> payload) throws DriverException {
        try {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                throw new DriverException("email relay HTTP " + response.statusCode() + ": " + detail);
            }
            ...
            throw new DriverException("email relay failed", e);
        }
    }
    ...
    private static void requireOutboundMapping(String raw) throws DriverException {
        if (raw == null || raw.isBlank()) {
            throw new DriverException("email point mapping is blank");
        }
        String token = raw.trim().toLowerCase(Locale.ROOT);
        if (!"outbound".equals(token) && !"send".equals(token) && !"email".equals(token)) {
            throw new DriverException("email point mapping must be outbound|send|email, got: " + raw);
        }
    }
