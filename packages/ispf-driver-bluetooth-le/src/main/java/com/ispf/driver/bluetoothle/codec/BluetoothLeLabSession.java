...
/**
 * TCP session for the Bluetooth LE GATT TCP gateway lab (default port {@code 9999}).
 * <p>
 * Honesty: newline JSON over TCP to a GATT gateway lab — not BLE radio / SoftDevice / HCI.
 * <p>
 * Dialect:
 * <pre>
 *   {"op":"get","point":"mac:AA:BB:CC:DD:EE:FF:svc:180f:char:2a19"}{@code \\n}
 *   {"op":"get","point":"device:1:rssi"}{@code \\n}
 *   {"op":"set","point":"mac:...:char:2a19","value":42}{@code \\n}
 * </pre>
 * Responses: {@code {"ok":true,"value":...}} / {@code {"ok":false,"error":"..."}}.
 */
public final class BluetoothLeLabSession implements AutoCloseable {
    ...
    public BluetoothLeLabSession(String host, int port, int timeoutMs) throws IOException {
        socket = new Socket();
    ...
    private String transact(String jsonLine) throws IOException {
        ...
        if (line == null) {
            throw new EOFException("EOF from Bluetooth LE GATT gateway lab");
        }
        return line;
    }
    ...
    static double parseValue(String response) throws IOException {
        ...
        if (!matcher.find()) {
            throw new IOException("Bluetooth LE GATT gateway lab response missing value: " + response);
        }
    ...
    static void ensureOk(String response) throws IOException {
        ...
        if (ok.find() && "false".equalsIgnoreCase(ok.group(1))) {
            ...
            throw new IOException("Bluetooth LE GATT gateway lab rejected: " + message);
        }
        if (response.toLowerCase(Locale.ROOT).contains("\"error\"")) {
            Matcher err = ERROR_FIELD.matcher(response);
            if (err.find()) {
                throw new IOException("Bluetooth LE GATT gateway lab rejected: " + err.group(1));
            }
        }
    }
