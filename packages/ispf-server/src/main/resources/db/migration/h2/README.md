...
PostgreSQL-only bodies (`V86__tenant_row_level_security.sql`,
`V89__random_uuid_compat.sql`, …) are wrapped in Flyway placeholders
(`${rls_block_start}` / `${rls_block_end}`) so H2 receives a block comment while
PostgreSQL executes the DDL (`FlywayDialectConfiguration`).
