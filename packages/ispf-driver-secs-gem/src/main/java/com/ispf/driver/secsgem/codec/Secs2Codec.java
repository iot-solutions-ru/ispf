...
/**
 * SECS-II item codec for the streams and functions used by this driver.
 * <p>
 * Supports List, ASCII, U1, U2, U4, Boolean, and F4.
 */
public final class Secs2Codec {
    ...
    public static Parsed parseAt(byte[] data, int offset) throws IOException {
        ...
        if (lengthBytes == 0) {
            return new Parsed(new Item(format, List.of(), null, 0L, 0.0, false), offset + 1);
        }
        ...
        if (format == FORMAT_LIST) {
            ...
            return new Parsed(new Item(format, children, null, 0L, 0.0, false), cursor);
        }
        ...
        return switch (format) {
            case FORMAT_ASCII -> new Parsed(
                    new Item(format, List.of(), new String(payload, StandardCharsets.US_ASCII), 0L, 0.0, false),
                    dataStart + length);
            case FORMAT_BOOLEAN -> {
                boolean value = length > 0 && payload[0] != 0;
                yield new Parsed(new Item(format, List.of(), null, value ? 1L : 0L, 0.0, value),
                        dataStart + length);
            }
            case FORMAT_U1 -> {
                long value = length > 0 ? payload[0] & 0xFFL : 0L;
                yield new Parsed(new Item(format, List.of(), null, value, value, false), dataStart + length);
            }
            case FORMAT_U2 -> {
                long value = length >= 2
            ...
            case FORMAT_F4 -> {
                ...
                yield new Parsed(new Item(format, List.of(), null, (long) value, value, false),
                        dataStart + length);
            }
            default -> new Parsed(new Item(format, List.of(), null, 0L, 0.0, false), dataStart + length);
        };
    }
