 ...
 * </ul>
 * Point mapping: {@code D100}, {@code M0}, {@code R10}. Full {@code @…*} frames may be sent as-is
 * (FCS recomputed when a trailing {@code **} placeholder is present).
 * <p>
 * Clean-room ISPF code, Apache-2.0 — JDK sockets only; no vendor SDK / PLC4X.
 */
public class IdecMicrosmartDeviceDriver implements DeviceDriver {

    private static final Pattern REGISTER = Pattern.compile(
            "^(?<dev>[DMR])(?<addr>\\d+)$",
            Pattern.CASE_INSENSITIVE);

    private static final DataSchema VALUE_SCHEMA = DataSchema.builder("idecMicrosmartValue")
    ...
    public void connect() throws DriverException {
        try {
            ...
            socket = next;
            connected = true;
            driverObject.log(DriverLogLevel.INFO, "IDEC MicroSmart Host Link connected to " + host + ":" + port);
        } catch (IOException e) {
            closeSocket();
            throw new DriverException("IDEC MicroSmart connect failed for " + host + ":" + port, e);
        }
    }
    ...
    public void readPoints(Map<String, String> pointMappings) throws DriverException {
        ...
        for (Map.Entry<String, String> entry : pointMappings.entrySet()) {
            ...
            if (isErrorResponse(response)) {
                throw new DriverException("IDEC MicroSmart read rejected: " + response);
            }
    ...
    public void writePoint(String pointId, DataRecord value) throws DriverException {
        ...
        String response = transact(command);
        if (isErrorResponse(response)) {
            throw new DriverException("IDEC MicroSmart write rejected: " + response);
        }
        driverObject.updateVariable(pointId, DataRecord.single(VALUE_SCHEMA, Map.of(
                "value", payload,
                "register", registerFromMapping(mapping),
    ...
    static String buildReadCommand(String station, String mapping) {
        ...
        if (map.isBlank()) {
            throw new IllegalArgumentException("Blank IDEC Host Link mapping");
        }
        if (map.startsWith("@")) {
        ...
        throw new IllegalArgumentException("Unsupported IDEC point mapping: " + mapping);
    }
    ...
    static String buildWriteCommand(String station, String mapping, String payload) {
        ...
        throw new IllegalArgumentException("Unsupported IDEC write mapping: " + mapping);
    }
    ...
    static String ensureFcsFrame(String frame) {
        ...
        if (!trimmed.startsWith("@")) {
            throw new IllegalArgumentException("IDEC Host Link frame must start with @: " + frame);
        }
        if (trimmed.endsWith("*")) {
    ...
    static String normalizeStation(String station) {
        ...
        throw new IllegalArgumentException("IDEC station must be 2 decimal digits: " + station);
    }
    ...
    private static String normalizeWriteData(String device, String value) {
        ...
        throw new IllegalArgumentException("IDEC write value must be numeric: " + value);
    }
    ...
    private synchronized String transact(String command) throws DriverException {
        try {
            ...
            throw new DriverException(
                    "IDEC MicroSmart I/O failed for " + host + ":" + port + " (" + command + ")", e);
        }
    }
    ...
    static String readFrame(InputStream in) throws IOException {
        ...
        while (true) {
            ...
            if (ch < 0) {
                if (line.size() == 0) {
                    throw new IOException("EOF reading IDEC Host Link response");
                }
                break;
            }
            if (ch == '\r' || ch == '\n') {
