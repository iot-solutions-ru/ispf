...
/**
 * CC-Link IE SLMP 3E device point.
 * <p>
...
record CcLinkIePoint(String deviceCode, int address) {
    ...
    static CcLinkIePoint parse(String mapping) throws DriverException {
        if (mapping == null || mapping.isBlank()) {
            throw new DriverException("CC-Link IE point mapping is blank");
        }
        ...
        if (!device.matches()) {
            throw new DriverException(
                    "Unsupported CC-Link IE mapping (expected D100, R0, W0, or dev:D100): "
                            + mapping);
        }
        String code = device.group(1).toUpperCase(Locale.ROOT);
        int address = Integer.parseInt(device.group(2));
        if (address < 0 || address > 65535) {
            throw new DriverException("CC-Link IE address out of range: " + address);
        }
