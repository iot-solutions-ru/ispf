...
public record SchneiderUmacPoint(int address, int count) {
    ...
    public static SchneiderUmacPoint parse(String raw) {
        if (raw == null || raw.isBlank()) {
            throw new IllegalArgumentException("Schneider UMAC point mapping is blank");
        }
        ...
        if (trimmed.contains(":")) {
            ...
            if (parts.length < 2 || parts.length > 3) {
                throw new IllegalArgumentException(
                        "Schneider UMAC point must be HR:address[:count] or MW:address[:count]");
            }
            String area = parts[0].trim().toUpperCase(Locale.ROOT);
            if (!"HR".equals(area) && !"HOLDING".equals(area) && !"MW".equals(area)) {
                throw new IllegalArgumentException(
                        "Schneider UMAC v0.1 supports HR/MW holding registers only, got: " + area);
            }
            int address = Integer.parseInt(parts[1].trim());
            int count = parts.length == 3 ? Integer.parseInt(parts[2].trim()) : 1;
            return new SchneiderUmacPoint(address, requirePositive(count));
        }
        Matcher mwMatcher = COMPACT_MW.matcher(trimmed);
        if (mwMatcher.matches()) {
            return new SchneiderUmacPoint(Integer.parseInt(mwMatcher.group(1)), 1);
        }
        return new SchneiderUmacPoint(Integer.parseInt(trimmed), 1);
    }

    private static int requirePositive(int count) {
        if (count < 1) {
            throw new IllegalArgumentException("Schneider UMAC point count must be >= 1");
        }
        return count;
    }
}
