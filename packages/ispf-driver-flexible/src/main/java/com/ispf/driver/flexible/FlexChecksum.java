...
final class FlexChecksum {
    ...
    static String verifyAndPayload(byte[] frame, String algorithm, String marker, int checksumLength)
        ...
        if (!"sum16-complement-hex".equalsIgnoreCase(algorithm)) {
            throw new DriverConfigurationException("Unsupported checksum algorithm: " + algorithm);
        }
        if (marker == null || marker.isBlank()) {
            throw new DriverConfigurationException("checksumMarker is required when checksumAlgorithm is set");
        }
        if (checksumLength <= 0) {
            throw new DriverConfigurationException("checksumLength must be positive");
        }
        byte[] markerBytes = marker.getBytes(StandardCharsets.US_ASCII);
        int markerIndex = indexOf(frame, markerBytes);
        if (markerIndex < 0) {
            throw new DriverPermanentException("Checksum marker not found: " + marker);
        }
        int checksumStart = markerIndex + markerBytes.length;
        if (checksumStart + checksumLength > frame.length) {
            throw new DriverPermanentException("Checksum field truncated in response");
        }
        ...
        try {
            ...
            throw new DriverPermanentException("Invalid checksum hex: " + checksumHex, e);
        }
        ...
        if (sum != 0) {
            throw new DriverPermanentException("Checksum verification failed");
        }
