...
- **Function script Object Query steps** — `FunctionScriptValidator` rejected `queryRows`, `scan_objects`, `for_each_row`, and `apply_query_patch` while `FunctionScriptEngine` already ran them, so saves that validate the script failed with "Unknown script step type". Those four types are now known and checked like the engine expects.
