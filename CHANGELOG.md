...
- **Function invoke `inputSchema`** — `DataRecordPayloadResolver` preferred a non-empty client schema over the descriptor, and an empty body became `DataRecord.empty` without checking required input fields. Invoke now projects rows onto the descriptor `inputSchema` (drop extras, coerce types, fail on missing required fields); client schema no longer replaces the contract.
