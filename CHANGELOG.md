...
- **Interrupting boundary timer work-queue rows** — after a `cancelActivity=true` boundary timer fired, the attached user task stayed `OPEN`/`CLAIMED` in the work queue while the instance had already left that task. Saving the instance now completes orphan open/claimed queue rows whose node is no longer pending.
