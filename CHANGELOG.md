...
- **BPMN due timers** — boundary and intermediate catch deadlines were registered but never fired on their own (only `POST .../instances/{id}/timer`). A leader-locked `WorkflowDueTimerScheduler` polls waiting instances (`ispf.workflow.timer-poll-ms`, default 2s); the manual endpoint remains for tests and forced fire.
