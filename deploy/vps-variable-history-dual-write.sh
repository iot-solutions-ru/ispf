#!/bin/bash
# Enable JDBC primary + ClickHouse secondary historian dual-write (BL-116).
set -euo pipefail
...
echo "=== Enable variable historian dual-write (JDBC + ClickHouse) ==="
upsert ISPF_VARIABLE_HISTORY_STORE jdbc
upsert ISPF_VARIABLE_HISTORY_DUAL_WRITE_ENABLED true
...
journalctl -u ispf-server -n 200 --no-pager \
  | grep -E 'ClickHouse dual-write secondary ready|DualWrite|ClickHouse variable history ready' \
  | tail -5 || true

echo "=== Done: ISPF_VARIABLE_HISTORY_DUAL_WRITE_ENABLED=true (store=jdbc) ==="
