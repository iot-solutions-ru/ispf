#!/bin/bash
# Switch ISPF variable historian to Scylla/Cassandra (ispf-scylla must be running).
set -euo pipefail
...
STORE="${ISPF_VARIABLE_HISTORY_STORE:-scylla}"
...
echo "=== Enable variable historian -> ${STORE} ==="
upsert ISPF_VARIABLE_HISTORY_STORE "$STORE"
upsert ISPF_VARIABLE_HISTORY_CASSANDRA_CONTACT_POINTS "$CONTACT_POINTS"
upsert ISPF_VARIABLE_HISTORY_CASSANDRA_PORT "$CASSANDRA_PORT"
upsert ISPF_VARIABLE_HISTORY_CASSANDRA_LOCAL_DATACENTER "$LOCAL_DC"
upsert ISPF_VARIABLE_HISTORY_CASSANDRA_KEYSPACE "$KEYSPACE"
upsert ISPF_VARIABLE_HISTORY_CASSANDRA_TABLE "$TABLE"
...
journalctl -u ispf-server -n 200 --no-pager \
  | grep -E 'Cassandra variable history ready|TimescaleDB hypertable skipped for variable_samples' \
  | tail -3 || true

echo "=== Done: ISPF_VARIABLE_HISTORY_STORE=${STORE} ==="
