...
import { isSecurityRolePath, isSecurityRolesRoot, isSecurityRoot } from "../security/securityRolePath";
import { isSecurityUserPath, isSecurityUsersRoot } from "../security/securityUserPath";
import {
  isAlertRulePath,
  isAlertRulesRoot,
  isCorrelatorPath,
...
import { isEventFilterPath, isEventFiltersRoot } from "../automation/eventFilterPath";
import { isFederationRoot } from "../federation/federationPath";
...
function resolveCatalogType(path: string, objectType?: ObjectType): ObjectType | null {
  ...
  if (EXACT_CATALOG_PATHS.has(path)) {
    const byPath: Record<string, ObjectType> = {
      ...
      "root.platform.mes": "CUSTOM",
      "root.platform.mes.work-orders": "CUSTOM",
      "root.platform.mes.lots": "CUSTOM",
