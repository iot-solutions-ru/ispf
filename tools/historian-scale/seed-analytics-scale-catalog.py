#!/usr/bin/env python3
"""Seed history-enabled virtual devices for analytics scale lab (BL-210 / Enterprise L).

Creates ``root.platform.devices.analytics-scale-lab.tag-NNNNN`` DEVICE objects with a
``temperature`` variable (historyEnabled=true).

Usage:
  python3 tools/historian-scale/seed-analytics-scale-catalog.py \\
    --base-url https://ispf.iot-solutions.ru \\
    --tags 1000 --batch 100 --workers 8

Enterprise L gate target: ``--tags 50000``.
"""
...
def delete_object(session: requests.Session, base_url: str, path: str) -> None:
    response = session.delete(
        f"{base_url}/api/v1/objects/by-path",
        params={"path": path},
        timeout=120,
    )
...
def enable_history(session: requests.Session, base_url: str, path: str) -> None:
    ...
    if created.status_code not in (409,) and "already exists" not in created.text.lower():
        raise RuntimeError(
            f"create variable {path}.{HISTORY_VARIABLE}: "
    ...
    for attempt in range(12):
        ...
        if "unknown variable" in enabled.text.lower() and attempt < 11:
            ...
            continue
        raise RuntimeError(
            f"enable history {path}.{HISTORY_VARIABLE}: "
            f"HTTP {enabled.status_code} {enabled.text[:200]}"
        )
...
def seed_one(base_url: str, token: str, parent: str, name: str, display: str) -> str:
...
def seed_tags(
    ...
    *,
    ...
    # FOLDER type is not allowed under devices; CUSTOM acts as a container.
        ...
        while chunk_start <= tag_count:
            ...
            for future in as_completed(futures):
                try:
                    ...
                    if len(errors) >= 5:
                        raise RuntimeError(
                            f"seed failed ({len(errors)} errors): {errors[0]}"
                        ) from exc
            print(f"  seeded {chunk_end}/{tag_count} (ok={completed})", flush=True)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Device count (Enterprise L gate: 50000)",
    )
    parser.add_argument("--batch", type=int, default=200)
    parser.add_argument("--workers", type=int, default=12, help="Parallel HTTP workers")
    parser.add_argument("--start-index", type=int, default=1, help="Resume from 1-based index")
    parser.add_argument(
        "--fresh",
        action="store_true",
        help="Delete analytics-scale-lab folder before seeding",
    )
    ...
    print(
        "Done. Count history-enabled vars (not /analytics/tags binding rules):\n"
        "  python3 tools/historian-scale/count-history-enabled.py "
        f"--base-url {base} --path-prefix {PARENT}\n"
        "Then run tools/historian-scale/analytics-scale-gate.sh with "
        "ISPF_ANALYTICS_BENCH_BASE_URL and ISPF_ANALYTICS_BENCH_SKIP_CATALOG_GATE=false.",
        flush=True,
    )
    return 0
