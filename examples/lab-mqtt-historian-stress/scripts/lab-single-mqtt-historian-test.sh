#!/usr/bin/env bash
# I-01 single-node: MQTT TELEMETRY_ONLY → Scylla historian.
set -euo pipefail
...
scylla_count() {
  local cql_out
  cql_out=$(scylla_cqlsh "SELECT COUNT(*) FROM ispf.variable_samples;")
...
run_emqtt_publisher() {
  ...
  local log_file="${DIR}/emqtt-pub-single-hist-$$.log"
...
echo "=== I-01 MQTT historian (single-node, Scylla) ==="
...
echo "Publisher: emqtt on ${ISPF_LAB_LOADGEN_SSH}"
...
sleep "${WARMUP}"
start_scylla=$(scylla_count)
start_flushed=$(historian_flushed_total)
start_mqtt_rx=$(mosquitto_sys_counter '$SYS/broker/messages/received')
echo "Measure start: scylla_samples=${start_scylla} flushed=${start_flushed} mosquitto_rx=${start_mqtt_rx}"
sleep "${PHASE}"
sleep 5

wait "${pub_pid}" 2>/dev/null || true
sleep 5
wait_historian_queue_drain 90 || echo "WARN: historian queue still elevated" >&2

end_scylla=$(scylla_count)
end_flushed=$(historian_flushed_total)
end_mqtt_rx=$(mosquitto_sys_counter '$SYS/broker/messages/received')

delta_scylla=$((end_scylla - start_scylla))
...
echo ""
echo "=== I-01 results ==="
echo "Scylla variable_samples delta: ${delta_scylla} (${rate_scylla} samples/s)"
...
FAIL=0
if [ "${delta_scylla}" -lt 40 ]; then
  echo "FAIL: Scylla samples delta ${delta_scylla} < 40" >&2
  FAIL=1
fi
